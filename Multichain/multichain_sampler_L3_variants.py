"""
Multi-Chain Gibbs Sampler for Layer 3 Variants

This module implements multi-chain MCMC sampling for Layer 3 variants:
...
# =============================================================================
# Prediction Functions for Layer 3 Variants
# =============================================================================

def predict_gp_variant_3layer_W_Known(X_new: np.ndarray, X_train: np.ndarray, Y_train: np.ndarray,
    ...
    """
    GP prediction for 3-layer W_Known variant.
    ...
    # For 3-layer, prediction is based on Q (not Z or R)
...
def predict_gp_variant_3layer_No_W(X_new: np.ndarray, X_train: np.ndarray, Y_train: np.ndarray,
                                  Q_train: np.ndarray,
                                  theta_y: Union[float, np.ndarray], g_y: float, tau2_y: float,
                                  kernel_type: str = 'separable_squared_exponential') -> Tuple[np.ndarray, np.ndarray]:
    """
    GP prediction for 3-layer No_W variant (X used directly).
...
def predict_gp_variant_3layer_No_W_Selective(X_new: np.ndarray, X_train: np.ndarray, Y_train: np.ndarray,
                                             Q_train: np.ndarray,
                                             theta_y: Union[float, np.ndarray], g_y: float, tau2_y: float,
                                             kernel_type: str = 'separable_squared_exponential') -> Tuple[np.ndarray, np.ndarray]:
    """
    GP prediction for 3-layer No_W_Selective variant (selected columns of X).
...
# =============================================================================
# Log-Likelihood Computation for Layer 3 Variants
# =============================================================================

def compute_loglikelihood_variant_3layer(samples: Dict, X_train: np.ndarray, Y_train: np.ndarray,
    ...
    """
    Compute log-likelihood for Layer 3 variant models.
    
    For 3-layer: BIC = loglik_y + loglik_q + loglik_r - 0.5 * k * log(n)
    
    Args:
        samples: Dictionary with tau2_y, g_y, theta_y, Q, R samples
        X_train: Training inputs (n_train, p)
        Y_train: Training responses (n_train,)
    ...
    Returns:
        Total log-likelihood (sum of Y, Q, and R layer log-likelihoods)
    """
    ...
    theta_r = np.mean(samples['theta_r'], axis=0) if samples['theta_r'].ndim > 1 else np.mean(samples['theta_r'])
    
    # Get Q and R (use mean across samples)
    Q_samples = samples['Q']
    ...
    R_samples = samples['R']
    if R_samples.ndim == 3:  # (n_samples, n, D)
        R = np.mean(R_samples, axis=0)  # (n, D)
    else:  # (n_samples, n) for D=1
        R = np.mean(R_samples, axis=0)  # (n,)
        if R.ndim == 1:
            R = R.reshape(-1, 1)
    ...
    if D == 1:
        ...
        kernel_r = get_kernel_instance(kernel_type, theta_r, 0.0, 1.0, D)  # g_r=0, tau2_r=1.0
    else:
        from parameter_sampler_Dgeneral import get_kernel_instance
        kernel_y = get_kernel_instance(kernel_type, theta_y, g_y, tau2_y, D)
        kernel_q = get_kernel_instance(kernel_type, theta_q, 0.0, 1.0, D)  # g_q=0, tau2_q=1.0
    ...
    try:
        ...
        # Q layer: Q | R
        loglik_q = kernel_q.log_likelihood(Q.flatten() if Q.ndim > 1 else Q, R)
        
        # R layer: R | Z
        loglik_r = kernel_r.log_likelihood(R.flatten() if R.ndim > 1 else R, Z)
        
        return loglik_y + loglik_q + loglik_r  # Sum for 3-layer BIC
    except:
        return -np.inf


# =============================================================================
# Bayesian Metrics for Layer 3 Variants
# =============================================================================

def Bayesian_Metrics_with_quantiles_variant_3layer(samples: Dict, X_train: np.ndarray, Y_train: np.ndarray,
    ...
    """
    Compute Bayesian metrics for Layer 3 variants.
    ...
    for i in range(n_samples):
        ...
        theta_r = samples['theta_r'][i] if samples['theta_r'].ndim == 1 else samples['theta_r'][i, :]
        ...
        if variant == 'W_Known':
            pred_mean, pred_var = predict_gp_variant_3layer_W_Known(
                X_test, X_train, Y_train,
                W_fixed, Q_i,
                theta_y, g_y, tau2_y, kernel_type
            )
        elif variant == 'No_W':
            pred_mean, pred_var = predict_gp_variant_3layer_No_W(
                X_test, X_train, Y_train,
                Q_i,
                theta_y, g_y, tau2_y, kernel_type
            )
        else:  # No_W_Selective
            pred_mean, pred_var = predict_gp_variant_3layer_No_W_Selective(
                X_test, X_train, Y_train,
                Q_i,
                theta_y, g_y, tau2_y, kernel_type
            )
        ...
        sample_i = {
            ...
            'theta_r': theta_r_arr if theta_r_arr.ndim == 1 and theta_r_arr.size == 1 else theta_r_arr[np.newaxis, :],
            'Q': samples['Q'][i][np.newaxis, ...],
            'R': samples['R'][i][np.newaxis, ...]
        }
        loglik_i = compute_loglikelihood_variant_3layer(
            sample_i, X_train, Y_train, variant, W_fixed, column_indices, kernel_type
        )
        k = 5  # tau2_y, g_y, theta_y, theta_q, theta_r
        n = len(Y_train)
...
# =============================================================================
# Multi-Chain Sampler for Layer 3 Variants
# =============================================================================

class MultiChainSampler_L3_Variants:
    """
    Multi-chain sampler for Layer 3 variants.
    ...
    def __init__(self, variant: str = 'No_W',
        ...
        """
        Initialize multi-chain sampler for Layer 3 variants.
    ...
    def run_chains(self, Y_train: np.ndarray, X_train: np.ndarray,
        ...
        for chain_idx in range(self.n_chains):
            if verbose:
                print(f"\n{'='*70}")
                print(f"Chain {chain_idx + 1}/{self.n_chains} - Variant: {self.variant} (Layer 3)")
                print(f"{'='*70}")
            ...
            # Compute metrics
            metrics = Bayesian_Metrics_with_quantiles_variant_3layer(
    ...
    def _compute_convergence_diagnostics(self) -> Dict:
        ...
        # For theta_r (R layer)
        theta_r_chains = [chain['theta_r'] for chain in self.chains_samples]
        if theta_r_chains[0].ndim == 0:
            r_hat_theta_r = self._gelman_rubin([c.flatten() for c in theta_r_chains])
        else:
            n_dims = theta_r_chains[0].shape[1] if theta_r_chains[0].ndim > 1 else 1
            if n_dims == 1:
                r_hat_theta_r = self._gelman_rubin([c.flatten() for c in theta_r_chains])
            else:
                r_hat_theta_r = np.array([self._gelman_rubin([c[:, d] for c in theta_r_chains]) 
                                        for d in range(n_dims)])
    ...
    def create_all_diagnostics(self, output_dir: str = './diagnostics'):
        ...
        # For theta_y, theta_q, theta_r, handle both scalar and vector cases
        ...
        theta_r_all_chains = [s['theta_r'] for s in self.chains_samples]
        if theta_r_all_chains[0].ndim == 0:
            plot_trace([c.flatten() for c in theta_r_all_chains], 'theta_r', output_dir)
        else:
            n_dims = theta_r_all_chains[0].shape[1] if theta_r_all_chains[0].ndim > 1 else 1
            if n_dims == 1:
                plot_trace([c.flatten() for c in theta_r_all_chains], 'theta_r', output_dir)
            else:
                for d in range(n_dims):
                    plot_trace([c[:, d] if c.ndim > 1 else c for c in theta_r_all_chains], 
                              f'theta_r_d{d}', output_dir)
        
        # Density plots (combine all chains)
        plot_density(tau2_all_chains, 'tau2_y', output_dir)
        plot_density(g_y_all_chains, 'g_y', output_dir)
        
        # Actual vs Predicted (skip for now - needs Y_test)
...
if __name__ == "__main__":
    print("="*70)
    print("Multi-Chain Sampler for Layer 3 Variants - Test")
    print("="*70)
    ...
    multichain1 = MultiChainSampler_L3_Variants(
        variant='W_Known',
        W_fixed=W_fixed,
        n_chains=2,
        n_iterations=5,
    ...
    multichain2 = MultiChainSampler_L3_Variants(
        variant='No_W',
        n_chains=2,
        n_iterations=5,
        burn_in=1,
    ...
    multichain3 = MultiChainSampler_L3_Variants(
